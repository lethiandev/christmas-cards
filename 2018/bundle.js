(function () {
	...
	var snowfalls = Array.from(document.getElementsByClassName('snowfall'));

	for (var _i2 = 0; _i2 < snowfalls.length; _i2++) {
	  var snowfall = snowfalls[_i2];
	  initSnowfall(snowfall);
	}

	var christmasTrees = Array.from(document.getElementsByClassName('christmas-tree'));

	for (var _i3 = 0; _i3 < christmasTrees.length; _i3++) {
	  var christmasTree = christmasTrees[_i3];
	  initChristmasTree(christmasTree);
	} // Book


	function initBook(book) {
	  var pages = Array.from(book.getElementsByClassName('page'));
	...
	function initBookPage(book, page, isEven) {
	  var flips = Array.from(page.getElementsByClassName('page-flip'));

	  for (var _i4 = 0; _i4 < flips.length; _i4++) {
	    var flip = flips[_i4];
	...
	function initBookPageFlipping(book, page, flip, previous) {
	  flip.addEventListener('click', function () {
	    var pages = Array.from(book.getElementsByClassName('page'));
	    var oddPages = pages.filter(function (page, idx) {
	      return !(idx % 2);
	    });

	    if (previous) {
	      var revealedPages = oddPages.filter(function (page) {
	        return page.classList.contains('page-revealed');
	      });

	      if (revealedPages.length) {
	        var prevPage = revealedPages.pop();
	        prevPage.classList.remove('page-revealed');
	        prevPage.classList.add('page-closed');
	      }
	    } else {
	      var closedPages = oddPages.filter(function (page) {
	        return !page.classList.contains('page-revealed');
	      });

	      if (closedPages.length) {
	        var nextPage = closedPages.shift();
	        nextPage.classList.remove('page-closed');
	        nextPage.classList.add('page-revealed');
	      }
	    }

	    if (oddPages.some(function (page) {
	      return page.classList.contains('page-revealed');
	    })) {
	...
	function initSnowfallDraw(canvas) {
	  ...
	  var pixelData = new Uint8Array([0, 0, 0, 0]);
	...
	function drawSnowfall(canvas, gl, rc) {
	  ...
	  var drawCount = rc.snowflakes.count;

	  if (window.innerWidth < 768) {
	    drawCount = rc.snowflakes.count * 0.5 | 0;
	  }

	  var vertices = rc.snowflakes.vertexData;

	  for (var i = 0; i < drawCount; i++) {
	    var sin = Math.cos(Date.now() * 0.001 + i);
	    vertices[i * 4 + 0] -= 0.2 / width + 0.3 / width * sin;
	    vertices[i * 4 + 1] -= 1.0 / height;
	    var rw = vertices[i * 4 + 2] * (1.0 / width);

	    if (vertices[i * 4 + 0] < -1.0 - rw) {
	      vertices[i * 4 + 0] = 1.0 + rw;
	    }

	    var rh = vertices[i * 4 + 2] * (1.0 / height);
	...
	function initChristmasTree(tree) {
	  tree.onload = function () {
	    return initChristmasTreeSVG(tree.contentDocument);
	  };
	}

	function initChristmasTreeSVG(svg) {
	  var ornaments = svg.getElementById('ornaments');
	  var ornamentElements = Array.from(ornaments.children);
	  var colors = ['FFEFB9', '941F1F', 'F4D02A', '6040AA', 'E47A1F', '9E001C', '4060F0'];

	  (function randomizeColors() {
	    setTimeout(randomizeColors, 1000);

	    for (var _i5 = 0; _i5 < ornamentElements.length; _i5++) {
	      var ornament = ornamentElements[_i5];
	      var color = colors[Math.random() * colors.length | 0];
	      ornament.style.fill = "#".concat(color);
	    }
	  })();
	}
